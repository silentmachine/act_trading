...
APCA_API_BASE_URL = os.environ["ALPACA_BASE_URL"]
...
class LongShort:
    ...
    def awaitMarketOpen(self):
        ...
        while(not isOpen):
            ...
            opening_time_local = datetime.datetime.fromtimestamp(openingTime, local_tz)
            current_time_local = datetime.datetime.fromtimestamp(currTime, local_tz)
            print(f"Current Time (Local): {current_time_local.strftime('%Y-%m-%d %H:%M:%S')}, Opening Time (Local): {opening_time_local.strftime('%Y-%m-%d %H:%M:%S')} Time to Open: {timeToOpen}")
            time.sleep(10)
            isOpen = self.alpaca.get_clock().is_open
    ...
    def submitOrder(self, qty, stock, side, resp):
        if(qty > 0):
            try:
                ...
                print(f"Order of | {qty} {stock} {side} | did not go through. Error: {e}")
                resp.append(False)
        else:
